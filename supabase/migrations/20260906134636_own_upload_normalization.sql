-- Self-source preparation is covered by the current store artifact. These
-- functions do not grant purposes, create analyses, or send report-ready mail.
alter table public.genome_files
 add column normalization_completed_at timestamptz,
 add column normalization_source_revision bigint,
 add constraint normalization_source_completion check (
  (normalization_completed_at is null and normalization_source_revision is null) or
  (normalization_completed_at is not null and normalization_source_revision is not null and normalization_source_revision=upload_revision
   and single_logical_sample_verified_at is not null));

-- Unpublished genetic batches remain private and file-owned, never visible
-- through the canonical row APIs. File deletion cascades through both tables.
create table private.own_normalization_runs (
 ...
 state text not null check(state in ('running','complete','failed','rejecting','rejected')),
 provenance jsonb
);
create table private.own_normalization_batches (
 file_id uuid not null references private.own_normalization_runs(file_id) on delete cascade,
 kind text not null check(kind in ('variants','observed')),
 sequence integer not null check(sequence>=0),
 rows jsonb not null check(jsonb_typeof(rows)='array' and jsonb_array_length(rows) between 1 and 1000),
 primary key(file_id,kind,sequence)
);
alter table private.own_normalization_runs enable row level security;
alter table private.own_normalization_batches enable row level security;
revoke all on private.own_normalization_runs,private.own_normalization_batches from public,anon,authenticated,inherit_upload_only;
insert into public.purge_target_stores(target_id,store_name,store_order) values
 ('variant-rows','private.own_normalization_batches',4),
 ('variant-rows','private.own_normalization_runs',5);

create function private.own_upload_normalization_v1(p_operation text,p_account_id uuid,p_session_id uuid,
...
$function$;
revoke all on function private.own_upload_normalization_v1(text,uuid,uuid,uuid,uuid,jsonb) from public,anon,authenticated,inherit_upload_only;
grant execute on function private.own_upload_normalization_v1(text,uuid,uuid,uuid,uuid,jsonb) to service_role;
create function public.own_upload_normalization_v1(p_operation text,p_account_id uuid,p_session_id uuid,
 p_file_id uuid,p_claim uuid default null,p_payload jsonb default null)
returns jsonb language sql security invoker set search_path=pg_catalog as $function$
 select private.own_upload_normalization_v1(p_operation,p_account_id,p_session_id,p_file_id,p_claim,p_payload);
$function$;
revoke all on function public.own_upload_normalization_v1(text,uuid,uuid,uuid,uuid,jsonb) from public,anon,authenticated,inherit_upload_only;
grant execute on function public.own_upload_normalization_v1(text,uuid,uuid,uuid,uuid,jsonb) to service_role;

-- Finite, database-selected crash cleanup. Source objects and completed rows
-- are untouched; only expired, unpublished normalization working data goes.
create function private.reap_expired_own_normalizations_v1()
returns integer language plpgsql security definer set search_path=pg_catalog,private as $function$
declare v_file uuid; r private.own_normalization_runs%rowtype; v_count integer:=0;
begin
 for v_file in select f.id from public.genome_files f join private.own_normalization_runs n on n.file_id=f.id
  where n.state='running' and n.expires_at<=clock_timestamp() and f.normalization_completed_at is null
   and not exists(select 1 from private.genome_file_deletions d where d.file_id=f.id)
  order by n.expires_at,f.id limit 5 for update of f skip locked
 loop
  select * into r from private.own_normalization_runs where file_id=v_file for update;
  if r.state<>'running' or r.expires_at>clock_timestamp() then continue; end if;
  delete from private.own_normalization_batches where file_id=v_file;
  update private.own_normalization_runs set state='failed' where file_id=v_file;
  update public.genome_files set status='failed',error='File preparation was interrupted. Please try again.'
   where id=v_file and processing_run_id=r.claim and status='parsing' and normalization_completed_at is null;
  v_count:=v_count+1;
 end loop;
 return v_count;
end;
$function$;
revoke all on function private.reap_expired_own_normalizations_v1() from public,anon,authenticated,inherit_upload_only;
grant execute on function private.reap_expired_own_normalizations_v1() to service_role;
create function public.reap_expired_own_normalizations_v1()
returns integer language sql security invoker set search_path=pg_catalog as $function$
 select private.reap_expired_own_normalizations_v1();
$function$;
revoke all on function public.reap_expired_own_normalizations_v1() from public,anon,authenticated,inherit_upload_only;
grant execute on function public.reap_expired_own_normalizations_v1() to service_role;
