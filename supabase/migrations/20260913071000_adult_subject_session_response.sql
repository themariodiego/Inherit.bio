-- D-081 step 2: one body answers the adult-subject invitation, whether the
-- authority arrives as the mailed token or as the rights session that token
-- opened.
--
-- The consent writing here is long — a subject rebinding, a principal
-- promotion, an account binding, a relationship, a signature, a consent
-- scope, and on refusal a contact shredding and a subject purge. Copying it
-- for the session path would leave two versions of the one transaction that
-- decides who controls a person's genome, and they would drift. So the body
-- moves into a private helper, and both entry points resolve their authority
-- and call it.
--
-- The helper resolves through `private.current_adult_subject_invitation_v1`,
-- which is stricter than the chain of selects it replaces: it also requires
-- the token revision, the invitation candidate revision and the contact
-- authority revision to agree, and the slot and principal to be the adult
-- ones. Every check it adds holds for an invitation that
-- `create_adult_subject_invitation_v1` made and the mail worker issued.
...
-- The mailed-token entry point. Unchanged in signature, result vocabulary
-- and behaviour; it now resolves through the shared authority and body.
-- It stays until the tokens already in people's mailboxes have expired.
...
begin
  ...
  return private.adult_subject_invitation_response_v1(
    v_token.id, null, p_action, p_account_id, p_account_email_hmac
  );
end;
$$;
...
-- The rights-session entry point. The browser presents the host-only cookie
-- and a form nonce bound to that session; no token, and nothing about the
-- invitation, is in the URL. The nonce is consumed before the invitation is
-- read, so one served form answers at most once.
create or replace function public.respond_adult_subject_invitation_session_v1(
  p_session_hash text,
  p_action text,
  p_nonce text,
  p_account_id uuid default null,
...
begin
  ...
  perform private.consume_embryo_operation_nonce_v1(
    p_nonce, p_account_id, null, 'invitation_respond',
    'rights_session', v_session.id
  );

  return private.adult_subject_invitation_response_v1(
    v_session.token_hash_id, v_session.id, p_action,
    p_account_id, p_account_email_hmac
  );
end;
$$;
