...
select is((select count(*) from private.invitation_terminal_notices),0::bigint,
...
select is(public.expire_invitation_terminal_notices_v1(),2,'fixed-deadline cleanup deletes both terminal recipient graphs');
select is((select count(*) from private.invitation_terminal_notices),0::bigint,'no terminal recipient envelope remains');
select is((select count(*) from public.mail_outbox where invitation_terminal_notice_id is not null),0::bigint,
