...
select * from public.activate_rights_session_v1(
  (select token_hash from tok), repeat('c', 64)
);
...
select is(
  (select count(*) from public.activate_rights_session_v1(
    (select token_hash from tok), repeat('d', 64))),
...
select is((select cards -> 0 ->> 'delivery_kind' from del), 'initial',
  'initial cards are labelled as such');
select throws_ok(
  $$select * from public.deliver_embryo_record_key_cards_v1(
      '7a000000-0000-0000-0000-000000000002',
      '7a000000-0000-4000-8000-0000000000b1',
...
select is(
  ...
  6::bigint, 'every commit notifies both notice recipients');
...
set fixed_expires_at = created_at + interval '1 second'
...
select is(
  (select status from public.subject_principals where id = (select principal_id from draft2_parent)),
  'revoked', 'the draft''s parent principal is revoked');
select is(
  (select count(*) from public.encrypted_contact_references
   where principal_id = (select principal_id from draft2_parent) and status = 'shredded'),
  1::bigint, 'the draft''s contact is shredded');
select is(
