...
| `scripts/readability-gate.test.ts` | New unit coverage | Prevents the static G1.10 extractor from inventing composite copy blocks or regressing remediated provider, report-template, privacy-policy, or GINA copy. | Proves nested copy containers are scored separately, inline markup stays inside its visible block, `alt`/`title` strings remain independently covered, and every displayed provider field plus all fourteen remediated template categories, the privacy policy, and the GINA explainer remains free of readability findings. |
