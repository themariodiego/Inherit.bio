...
| `scripts/readability-gate.test.ts` | New unit coverage | Prevents the static G1.10 extractor from inventing composite copy blocks or regressing remediated provider, report-template, privacy-policy, GINA, deceased-account, or law-enforcement copy. | Proves nested copy containers are scored separately, inline markup stays inside its visible block, `alt`/`title` strings remain independently covered, and every displayed provider field plus all fifteen report categories, the privacy policy, GINA explainer, deceased-account policy, and law-enforcement policy remains free of readability findings. |
