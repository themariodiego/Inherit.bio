...
| --- | --- | --- | --- |
| `src/app/api/files/[id]/route.test.ts`, `supabase/tests/genome_file_deletion.sql`, `e2e/file-deletion.spec.ts` | New file-deletion regression coverage | The old button ignored denied client Storage/database writes. | Tests preserve client privilege revocation while proving owner/live-session/exact-source authorization, retry on Storage failure, source ACK before cascades, processing and worker fences, visible failure, actual synthetic Storage deletion and retained self subject. No prior assertion is removed. |
...
| `supabase/tests/embryo_ingest_chunks.sql` | New database coverage | E0 per-ordinal persistence and retry boundary | 62 assertions cover distinct ordinal objects, stable retry IDs, single quota charges, idempotent commits, gaps/conflicts, malformed manifests, byte/line/record/chunk ceilings, stale and revoked auth, changed basis/due envelope, closed privileges and preserved failure targets. These are primitive tests, not a full upload or unwind proof. |
| `supabase/tests/embryo_ingest_chunks.sql` | Fixture strengthened; all 62 original assertions retained | Strict fragment path binding requires a genuinely issued upload identity and resolved format/build | Each normal fixture now uses actual draft, parent signatures, invitation activation/acceptance, finalization and session mint RPCs. Unique account pairs preserve the real per-account capacity guard. Known format/build is explicit synthetic setup, not a production decision bypass. The missing-auth-revision case starts as an intentionally legacy-shaped row instead of mutating an immutable minted session. No Storage calls, trigger bypasses or invented legacy object paths. The complete local suite passes 655 assertions across 15 files. |
| `supabase/tests/v2_contracts.sql` | Registry coverage extended | The new chunk-receipt store must participate in attempt cleanup | Count increases from 108 to 109; an additional assertion names `public.embryo_ingest_chunks` and requires its exact `upload-and-ingest-working-state` classification. No original assertion is removed. |
| `supabase/tests/embryo_cohort_runtime.sql`, `supabase/tests/fixtures/embryo_cohort_pre_finalize.inc` | Shared fixture extraction | Session finalization needs the same real parent/artifact fixture | Original fixture extracted verbatim, independently verified; every one of the original 141 assertions remains and passes. |
| `supabase/tests/embryo_ingest_sessions.sql` | 90 pgTAP assertions including 38 shared fixture assertions | Atomic session lifecycle and exact authority binding | Tests late due-store rollback, same-nonce retry, fixed deadline, opaque credential/handle hashes, immutability, capacity retained by expired/failed work, all-set/artifact/attestation drift, contradiction refusal, exact recipient/disposition due fields, closed privileges and retriable 55P03 rather than durable failure. No full upload, worker or unwind proof is claimed. |
| `supabase/tests/embryo_ingest_unwind.sql` | 72 pgTAP assertions including all 38 unchanged shared legal assertions | Frozen failed-attempt planning and independent terminal contact lifetime | Proves exact paths before upload, unresolved format/build refusal, original revoked recipient identity versus replacement revision refusal, unknown-store rollback, exact due target/recipient authority, inclusion of unacknowledged fragments, immutable manifest paths, same-plan retry and unchanged deadline. Proves no premature notice/purge, no terminal principal FK, service-only claims, fixed expiry, exact claim ACK and ciphertext deletion on persisted acceptance or expiry after exhausted attempts. No Storage drain, final graph purge or notice-producer proof is claimed. |
| `supabase/tests/v2_contracts.sql` | Additive registry coverage | Two internal unwind planning stores require explicit classification | Registered store count increases 109 to 111 and plan 18 to 20; both new stores are asserted under `upload-and-ingest-working-state`. All original assertions remain. |
| `src/lib/embryo/terminal-mail.test.ts`, `src/app/api/jobs/mail/route.test.ts`, `src/app/api/jobs/retention/route.test.ts`, `src/emails/emails.test.ts` | New and additive unit coverage | Independent terminal delivery must not retain plaintext or starve existing workflows | Proves data-free fixed notice, exact claim/provider idempotency, no plaintext in RPC/logs, no failure overwrite of uncertain accepted ACK, and continued ordinary mail/invitation/draft/account retention after terminal-queue errors. Existing render assertions remain. |
...
Adds `supabase/tests/export_archive_history_reader.sql`: 50 rollback-only
...
legacy consents, demographics and recipient grants directly. They are ordinary
...
for the recipient grant's foreign key. Four planted regressions each fail it.
