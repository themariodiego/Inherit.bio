...
| `scripts/readability-gate.test.ts` | New unit coverage | Prevents the static G1.10 extractor from inventing composite copy blocks or regressing remediated provider, lifestyle-template, brain-health-template, gastrointestinal-template, longevity-template, mental-health-template, basic-traits-template, or addiction-template copy. | Proves nested copy containers are scored separately, inline markup stays inside its visible block, `alt`/`title` strings remain independently covered, and every displayed provider field plus all seven remediated template categories remains free of readability findings. |
