-- Guarded production apply of supabase/migrations/20260923123240_export_archive_persistence.sql
-- (merged in #210; file SHA-256 2887fcd1ae351480b076cd0e84f922c3894f946ef69b8a0903e7ec16a042a6c5).
...
do $inherit_export_do$
declare
  migration constant text := $inherit_export_migration$-- Durable archive metadata. No worker, Storage write, route or scheduler is
...
$inherit_export_migration$;
begin
  if md5(migration) <> 'a72e462985670055e5fe14589a023a87' then
    raise exception using message = 'integrity: the embedded migration text is not the reviewed file'; end if;
  if exists (select 1 from supabase_migrations.schema_migrations where version = '20260923123240') then
    raise exception using message = 'predecessor: 20260923123240 is already in the ledger'; end if;
  if not exists (select 1 from supabase_migrations.schema_migrations where version = '20260923141529') then
    raise exception using message = 'predecessor: the 23 September guarded release row is missing'; end if;
  if to_regclass('private.export_archive_jobs') is not null or exists (select 1 from storage.buckets where id = 'exports') then
    raise exception using message = 'predecessor: export objects already exist'; end if;
  if md5(pg_get_functiondef('private.own_export_source_v1(uuid,uuid,uuid)'::regprocedure)) <> '27abc582be0eb4a227a7cdeb57c29d6f' then
    raise exception using message = 'predecessor: private.own_export_source_v1 differs from the tested definition'; end if;
  if (select count(*) from pg_constraint where conrelid = 'public.generated_exports'::regclass and contype = 'c'
      and pg_get_constraintdef(oid) = 'CHECK (((status = ''ready''::text) = ((completed_at IS NOT NULL) AND (expires_at IS NOT NULL) AND (object_id IS NOT NULL) AND (archive_sha256 IS NOT NULL) AND (manifest_sha256 IS NOT NULL) AND (byte_count IS NOT NULL))))') <> 1 then
    raise exception using message = 'predecessor: the single-object ready constraint is not exactly as expected'; end if;
  ...
  if md5(pg_get_functiondef('private.export_archive_authority_v1(jsonb,text,uuid)'::regprocedure)) <> 'c8b8b804c6ce535cbfd27a65dd49dbd5' then
    raise exception using message = 'postcheck: private.export_archive_authority_v1(jsonb,text,uuid) differs from the tested definition'; end if;
  ...
  if md5(pg_get_functiondef('private.export_archive_nonce_v1(uuid,text,jsonb,text)'::regprocedure)) <> '6a18866c7de349767fa94ea7f64a8af5' then
    raise exception using message = 'postcheck: private.export_archive_nonce_v1(uuid,text,jsonb,text) differs from the tested definition'; end if;
  if md5(pg_get_functiondef('private.guard_export_archive_delete_v1()'::regprocedure)) <> 'a4ad6defaa419d2823971fd7428c65f0' then
    raise exception using message = 'postcheck: private.guard_export_archive_delete_v1() differs from the tested definition'; end if;
  if md5(pg_get_functiondef('private.guard_segmented_export_publication_v1()'::regprocedure)) <> 'd27fa2fe535490231c004b8acd9e9ea8' then
    raise exception using message = 'postcheck: private.guard_segmented_export_publication_v1() differs from the tested definition'; end if;
  if md5(pg_get_functiondef('public.export_archive_cleanup_v1(text,uuid,uuid,jsonb)'::regprocedure)) <> '25fa3ce71b73fb0c371d1bb9af836bd7' then
    raise exception using message = 'postcheck: public.export_archive_cleanup_v1(text,uuid,uuid,jsonb) differs from the tested definition'; end if;
  if md5(pg_get_functiondef('public.export_archive_due_v1(text,uuid)'::regprocedure)) <> 'f554288a7efdec6bf8ad9a6956d11039' then
    raise exception using message = 'postcheck: public.export_archive_due_v1(text,uuid) differs from the tested definition'; end if;
  if md5(pg_get_functiondef('public.export_archive_request_v1(text,jsonb,text,uuid,jsonb,text)'::regprocedure)) <> 'f22a82e34511040aab5f8b8f68f671d0' then
    raise exception using message = 'postcheck: public.export_archive_request_v1(text,jsonb,text,uuid,jsonb,text) differs from the tested definition'; end if;
  if md5(pg_get_functiondef('public.export_archive_worker_v1(text,uuid,uuid,text,jsonb)'::regprocedure)) <> '9b3876aa3868ff30360b852ab1dc6fa3' then
    raise exception using message = 'postcheck: public.export_archive_worker_v1(text,uuid,uuid,text,jsonb) differs from the tested definition'; end if;
  if (select string_agg(c.relname, ',' order by c.relname) from pg_class c join pg_namespace n on n.oid = c.relnamespace
      where n.nspname = 'private' and c.relname like 'export_archive%' and c.relkind = 'r')
     is distinct from 'export_archive_attempts,export_archive_downloads,export_archive_jobs,export_archive_manifest_pages,export_archive_nonce_uses,export_archive_segments' then
    raise exception using message = 'postcheck: export tables differ'; end if;
  if (select string_agg(tgname, ',' order by tgname) from pg_trigger where tgrelid = 'public.generated_exports'::regclass and not tgisinternal)
     is distinct from 'guard_export_archive_delete,guard_segmented_export_publication' then
    raise exception using message = 'postcheck: generated_exports triggers differ'; end if;
  if md5((select pg_get_constraintdef(oid) from pg_constraint where conrelid = 'public.generated_exports'::regclass
      and conname = 'generated_exports_ready_representation')) is distinct from '8c2d030e2415d41b1d9b17bddb18efc3' then
    raise exception using message = 'postcheck: ready representation constraint differs'; end if;
  if not exists (select 1 from storage.buckets where id = 'exports' and public = false) then
    raise exception using message = 'postcheck: exports bucket missing'; end if;
  if has_function_privilege('authenticated', 'public.export_archive_request_v1(text,jsonb,text,uuid,jsonb,text)', 'execute')
     or has_function_privilege('anon', 'public.export_archive_worker_v1(text,uuid,uuid,text,jsonb)', 'execute') then
    raise exception using message = 'postcheck: a browser role can execute an export RPC'; end if;

  insert into supabase_migrations.schema_migrations (version, name, statements)
  values ('20260923123240', 'export_archive_persistence', array[migration]);
