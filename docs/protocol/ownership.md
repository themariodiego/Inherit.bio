...
| `src/lib/embryos/{operation-token,rights-session,basis,record-key-cards,api}.ts`, `src/app/api/embryo-cohort-drafts/`, `src/app/api/embryo-cohorts/`, `src/app/api/embryos/`, `src/app/api/cohorts/`, `src/app/api/invitations/`, `src/app/api/rights/`, `supabase/tests/embryo_cohort_runtime.sql` | Embryo Analysis | E0 cohort runtime: tokens, rights sessions, basis vocabulary, Record Key Cards, closed responses and the routes over the cohort RPCs |
| `content/legal/` | Legal | the artifact bodies the migration seeds verbatim; `legal-content.test.ts` keeps file and seed equal |
| `src/emails/{co-parent-invitation,embryo-upload-notice,record-key-addendum,embryo-disposition-notice,cohort-restriction-notice,embryo-draft-expired}.tsx` | Embryo Analysis | the six embryo notices |
