...
**Embryo source chromosome retention — owner revision, 22 September 2026.** Retain observed DNA calls on chromosomes 1–22, X and Y in each embryo's sanitized source and authorized source export. Normalize X/Y and numeric 23/24 aliases, preserve the reported allele calls and ploidy, and do not invent missing calls. Continue to exclude mitochondrial, unplaced, unknown and unresolved PAR contigs. Both transport parsers and the database enforce this source allow-list. This supersedes the earlier instruction to discard X/Y rows; it does not authorize inferring or presenting a sex or karyotype result. Neutral sample ordinals, removal of source labels and demographic columns, consent and access checks, and result-shape restrictions remain. Build inference and `embryo_qc.autosomal_het_rate` still compare chromosomes 1–22 only; stored X/Y calls do not become inputs to an autosomal model. Tests must prove X/Y round trips and reject unsupported contigs through browser transport, server validation and database boundaries.
