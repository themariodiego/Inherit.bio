...
**Embryo chunk authentication (owner decision, 2026-09-22).** Only `PUT /api/embryo-ingest/[session]/chunks/[sequence]` omits `X-Inherit-CSRF` and `X-Inherit-Chunk-Nonce`. Before reading bytes it requires the live authenticated account and originating auth session, a matching host-only `HttpOnly` `SameSite=Strict` upload-session cookie (`Secure` and `__Host-` prefixed in production), exact same Origin, and the jurisdiction guard. Chunk reservation and commit enforce the session, sequence, actual byte count and content hash; an identical retry is idempotent, and a changed hash at the same sequence fails the session. Fetch metadata remains subject to the shared same-origin guard. This is not an exception for mapping, completion, cancellation, or legal-evidence chunks. ADR 0020 and corrections item 20 record the rationale.

**Where the PDF refusal happens.** ADR-0016 sends every ordinary-subject upload browser → Storage directly, under the capability `docs/route-register.json` registers, which that ADR names the single authority for genetic-file transport: `POST /api/uploads` issues a one-use, short-lived bearer for a server-named staging key, and the browser then sends the file itself to Storage in a single request. So no file byte transits a function, and a byte-level rejection in `POST /api/uploads` is unreachable — not because the route is absent, but because `readDeclaration` in `src/lib/uploads/subject-upload-issuance.ts` accepts at most 4,096 bytes of JSON and never the file or a filename. (An earlier version of this paragraph cited the superseded ADR-0001, said the transport was TUS in 6 MiB chunks, and said `POST /api/uploads` did not exist. ADR-0016 rejects the TUS claim by name and the route is in the tree; corrected 21 September 2026. The transport matters beyond bookkeeping: a single standard-upload request carries about 5 GB where TUS carries 50 GB, which is why no admission ceiling may be set above that boundary. That is not left to configuration: `SINGLE_REQUEST_MAXIMUM_BYTES` in `src/lib/uploads/subject-upload-contract.ts` clamps every configured ceiling, and issuance refuses an oversized declaration before any durable row exists — see `docs/hosted-own-upload-readiness.md`.) Refusal therefore lives in the two places that see the bytes: (1) `sniffFile` in `src/lib/genome/parsers/sniff-browser.ts` detects `%PDF-` **before any byte reaches Storage** and blocks the upload; (2) `POST /api/files/[id]/process` re-sniffs the first 64 KiB server-side for any file already in Storage, sets `genome_files.error = 'pdf_not_data'`, returns HTTP 415 and deletes the object. `e2e/pdf-rejection.spec.ts` asserts both. No OCR path, no "estimate from the PDF" path, ever. Named anti-pattern (ADR-0008): **conclusion-laundering** — re-presenting another laboratory's printed conclusions as though Inherit had computed them.
...
- **Test hooks.** The browser suite advances queues by posting to the job routes themselves — `POST /api/jobs/mail`, `/api/jobs/retention`, `/api/jobs/research-publish` and `/api/jobs/research-refresh` — each authenticated by `JOBS_SECRET` and each draining the work that is due. Without them, purge and cohort tests would need 24-hour and 5-minute wall-clock waits inside Playwright and would be marked skipped. (Corrections item 12, signed 2026-09-13: this replaces a single `POST /api/jobs/run?kind=…` hook. That route was never built, no code has ever called it, and the two specs named here as its users — `e2e/revocation.spec.ts` and `e2e/embryo-ingest.spec.ts` — have never existed in this repository.)
...
- **G2.2** Every route declares and renders, with a named E2E test each: `empty`, `processing`, `partial-coverage`, `complete`, `not-covered`, `error`, `consent-required`, `jurisdiction-unavailable`. A route that structurally cannot enter a state declares `n/a` with a one-line reason naming the structural property, never an implementation choice. A route declares the `error` state only where a fault is reachable; routes that answer bad input with `notFound()` do not declare it, and the reason may name that convention rather than a structural property (operator decision, 2026-09-12 — the one exception to the sentence above). Four `n/a` declarations are forbidden outright: `not-covered` and `partial-coverage` on any route rendering a result derived from an uploaded file; `consent-required` on any Family or Embryo Analysis route that renders a result, a record or a permission of a named person; `jurisdiction-unavailable` on any route whose capability appears in `data/jurisdictions.json`. The `consent-required` prohibition does not reach three cases, and a route claiming one of them must record in the register which it claims and what carries the refusal instead: a route whose consent is a property of an item it lists rather than of the page; the route on which that consent is given; and a route that reads no consent at all. (Operator decision, 2026-09-13, on the measurement in corrections item 13 and D-108: the prohibition exists to stop a Family or Embryo route quietly declaring that consent does not apply to it, which it does wherever consent is a property of the page. Where it is not, the blanket forced a permanent unproven pair and invited someone to satisfy it by building a refusal nobody asked for.)
...
- **G3.2 — the task set.** Ten tasks, each bound in `docs/comprehension-protocol.md` to a named seed account, a named uploaded fixture, and the exact surface and content that count as success — an unbound task cannot be graded, because whether the right answer is a result or an honest not-covered state would be undetermined.
  ...
  - **T9** "Someone has uploaded your DNA. Starting from inherit.bio with no account, find out what is held about you, and delete it." Maximum 6 in-app actions, using G2.4's click and submit events and its registered confirmation exclusions. Record each mailed-link or typed-URL entry separately; entry is not counted toward this ceiling. (Owner decision, 2026-09-22.)
...
- **G5.6 — free export, correctly scoped.** Export is free and complete for every data class, proven by E2E with a privileged re-query showing zero rows and zero storage objects after deletion. **Scope**: the account holder's export contains their own genome and their own derived results in full; for a third-party subject or an embryo it contains only the consent record, the file metadata, and the derived results the subject's consent covers — **never another subject's raw sequence**, unless that subject has separately granted raw-data export. The third-party subject has their own independent export route for their own raw data. An E2E assertion opens the account holder's export archive and asserts zero raw variant rows attributable to another subject. Without this scoping, "complete free export" would hand one person another person's genome permanently, beyond any later revocation.
  ...
  **Large-export storage and action forms.** A complete export is one logical ZIP64 archive and one user download. Its bytes may occupy ordered, separate whole private Storage objects, each at most the registered 4,000,000-byte delivery chunk size. This changes the internal single-object representation, not the required data classes, privacy scope, total archive hash or 24-hour retention deadline. Every reserved object, including an uncertain write, belongs to the same durable cleanup obligation. The server must verify the complete member set and manifest before publishing ready, then authorize and verify each bounded chunk before delivery. It must not omit members or impose a silent archive-size cap to fit one provider object. The exact manifest and storage-key contract is `largeExportDeliveryContract.archiveRepresentation` in `docs/route-register.json`. This is neither provider multipart upload nor a change to original-genome storage. Authorized action forms may render stateless operation-specific nonces after a read-only authority check; only an explicit POST may consume one and create an export or download session. Poll responses keep their existing closed shape, and GET never creates or rotates those rows, sessions or cookies. This clarification is a required implementation contract, not evidence that G5.6 passes. (Owner decision, 2026-09-25.)
