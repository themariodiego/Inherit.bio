...
- Route-state matrix, 18 September (evening): six My Genome pairs proven in a
  ...
  and one case in `e2e/report-skeleton.spec.ts`); then two Family pairs,
  19 → 17 (`/family/[person] complete` in `e2e/family-health-picture.spec.ts`,
...
| D-098 | `NEXT_PUBLIC_APP_URL` falls back to the hosted origin. Should it fail loudly instead? | **Answered and closed 2026-09-11 — it did not turn on what production sets after all.** The two deployments are distinguishable without reading either one's configuration, and the fallback carries opposite risk on each. On the hosted deployment it is *correct*: `docs/route-register.json` pins `canonicalOrigin` to the same literal, so an unset value there produces exactly the origin the register already declares, and throwing would take out mail delivery to fix nothing. Anywhere else it is the defect, because the links it builds carry rights tokens. So the rule is the deployment, not the value: `src/lib/app-origin.ts` keeps the fallback where `VERCEL`/`VERCEL_ENV` are set and refuses an unset value everywhere else, at the point a link is built rather than at startup — a refusal that stops one message is recoverable, a message already sent is not. The literal also stopped being duplicated: two call sites hardcoded it, both now read one function, and a test fails if it drifts from the register. |
