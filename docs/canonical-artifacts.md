...
| Genetic-ingest rejection IDs and user-facing messages | `src/copy/upload/errors.ts` | file processor, embryo ingest, per-embryo QC reasons (`src/copy/embryos/qc.ts` spreads the halves), the uploader's browser preflight, `src/lib/genome/ingest-errors.ts` (the brief's path, a re-export), API/RSC states and `src/copy/upload/errors.test.ts`; every A.6 code is defined once here |
