...
| Embryo browser rewrite and server chunk validation | `docs/route-register.json#policyResolvers.embryo-ingest-session-v1.transportWireV1` | `src/lib/embryos/ingest-binding.ts`, `ingest-lines.ts`, `vcf-transport.ts`, `table-transport.ts`; route integration and whole-cohort publication remain separate prerequisites |
...
| Embryo chunk receipt, ordinal fragment reservation and durable failure marker | `supabase/migrations/20260905191141_embryo_ingest_chunk_reservations.sql`, implementing `docs/route-register.json#policyResolvers.embryo-ingest-session-v1` | Private persistence primitives and `supabase/tests/embryo_ingest_chunks.sql`; full authority resolution, session minting, Storage verification and unwind dispatch must precede route integration. These routines do not enable ingest. |
...
| Report method context, source-read dates and resolver-call accounting | `src/lib/genome/report-evidence.ts`; `src/copy/reports/basis.ts` owns copy; `src/components/reports/report-evidence.tsx` and the existing report-detail route render it after the established gates |
