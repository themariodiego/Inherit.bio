...
| G1.6 | Extended RLS attack suite covers every new table and revocation. | NO | `e2e/rls.spec.ts` does not yet cover all tables in `supabase/migrations/20260831*.sql`; adult invitation database invariants are narrower in `supabase/tests/adult_subject_invitation.sql`. Measured 2026-09-10 across every `enable row level security` in `supabase/migrations/`: 85 distinct tables, 66 in `public`. **20 of the 66 are named in no pgTAP test and not in `e2e/rls.spec.ts`**, and what they are matters. Fourteen have RLS enabled with **no policy at all**, which denies every non-bypass role by default — `abuse_events`, `account_security_states`, `ancestry_regions`, the three `audit_principal*` tables, `download_ranges`, `download_sessions`, `generated_exports`, `legal_audit_retention_checkpoints`, `rate_limit_hmac_buckets`, `ref_region_releases`, `ref_regions`, `research_releases`. The other six carry exactly one `*_public_read` policy over reference data: `providers`, `prs_weights`, `ref_aims`, `ref_genes`, `ref_haplogroup_markers`, `ref_variants`. **No untested table has a permissive user-scoped policy**, so the missing coverage is real but is not an open path from one person's data to another. Limits of the check: it reads `create policy` statements statically (only `alter policy` appears elsewhere, on the already-covered `profiles` and `genome_files`), and RLS is not the only control — `service_role` bypasses it, so a route using the admin client reaches these tables by design. The slice that can be enforced now is: `scripts/rls-policy-coverage.test.ts` requires every public table whose policy reads `auth.uid()` or `auth.jwt()` to be named by a test, which is 10 of 10 today and fails when the next permissive policy arrives without one — verified by planting such a policy on an uncovered table. It cannot judge whether the naming test attacks the policy; that is the rest of this row. |
