...
| G1.10 | Readability and vocabulary gate passes with self-test. | NO | `pnpm gate:readability` runs the pinned ten-case self-test, scans 1,489 extracted blocks (942 long, 397 short-role, 252 sentence-capped) using `data/plain-vocabulary.json` and `data/jargon.json`, and exits 0 on the whole corpus as of 2026-09-03; it is now a required step in `.github/workflows/ci.yml`. The row stays NO until the extractor provably covers runtime-generated copy (template literals, copy arrays/objects, status and email text, chart axis labels) as G1.10 requires; see `docs/readability-audit.md`. |
