...
| G1.10 | Readability and vocabulary gate passes with self-test. | NO | `pnpm gate:readability` runs the pinned ten-case self-test and scans 1,440 extracted blocks using `data/plain-vocabulary.json` and `data/jargon.json`, but exits non-zero on 406 long-block grade findings after the application and public-page remediation passes. See `docs/readability-audit.md`; CI is intentionally unchanged until the corpus is clean. |
