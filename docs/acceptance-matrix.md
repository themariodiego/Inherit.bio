...
| G1.4 | Unit suite and required pure-module coverage pass. | YES | `pnpm test` (160 tests); unit specs under `src/lib/**/*.test.ts` and `scripts/**/*.test.ts`. |
...
| G1.10 | Readability and vocabulary gate passes with self-test. | NO | `pnpm gate:readability` runs the pinned ten-case self-test and scans 1,440 extracted blocks using `data/plain-vocabulary.json` and `data/jargon.json`, but exits non-zero on 41 long-block grade findings after the application, public-page, provider-metadata, all fifteen report-template categories, privacy-policy, and GINA-explainer remediation passes. See `docs/readability-audit.md`; CI is intentionally unchanged until the corpus is clean. |
