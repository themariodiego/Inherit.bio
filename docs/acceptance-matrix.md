...
- PR72 released four reviewed everyday-report updates and three additional
  ...
  G1.11/G4.7 remain NO and the count stays 18/65.
...
| G2.3 | Every pre-existing route has a verified kept/redirect/gone disposition. | NO | Required `gate:routes` is absent; `docs/route-register.json` is not fully live-verified. Measured 2026-09-09 by matching register paths against `page.tsx` and `route.ts` under `src/app` (route groups stripped): the register and the tree diverge in **both** directions — **7** surfaces exist unregistered (`/api/files/[id]`, `/api/uploads`, `/api/uploads/[id]/complete`, `/api/withdraw`, `/api/withdraw/session`, `/withdraw/request`, `/withdraw/session`) and **52** registered endpoints have no handler file. All 10 `redirect` entries reconcile, including the three declared in `next.config.ts`. Most of the 52 are unbuilt Future Person, appeals, subjects, embryo-ingest and review routes, but the withdraw pair is different in kind: `/api/withdraw/[token]` and `/api/withdraw/[token]/export` are registered while the shipped flow is session-based and unregistered, so a live rights flow does not match its register. A gate cannot be written against this until each of the 52 is decided to be a commitment or a plan; that decision is this row's work, not the command. This is static file existence only — there is no `src/middleware.ts`, and an endpoint served by a server action would not be detected. |
...
| G8.5 | Every live surface/endpoint/prefix matches its register. | NO | Required comprehensive route/form/API/storage register verification is absent, and the gap is now measured rather than assumed: 7 surfaces under `src/app` have no register entry and 52 registered endpoints have no handler file (see G2.3 for the method, the exact paths and the limits of a static check). Storage prefixes and forms remain unverified separately. |
