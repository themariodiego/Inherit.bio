...
| G1.4 | Unit suite and required pure-module coverage pass. | YES | `pnpm test` (165 tests); unit specs under `src/lib/**/*.test.ts` and `scripts/**/*.test.ts`. |
| G1.5 | E2E has zero failures, skips, retries, or quarantine. | YES | `pnpm e2e` (58 tests); `playwright.config.ts` has `retries: 0`; `scripts/run-e2e.ts` validates `test-results/results.json`; `docs/test-diff-register.md`. |
...
| G1.10 | Readability and vocabulary gate passes with self-test. | NO | `pnpm gate:readability` runs the pinned ten-case self-test and scans 1,467 extracted blocks using `data/plain-vocabulary.json` and `data/jargon.json`, but exits non-zero on 19 long-block grade findings after the application, public-page, provider-metadata, all fifteen report-template categories, privacy-policy, GINA-explainer, deceased-account-policy, law-enforcement-policy, research-consent-policy, Copilot-setup, and appeals-policy remediation passes. See `docs/readability-audit.md`; CI is intentionally unchanged until the corpus is clean. |
