...
| A13 | Export ZIP has originals + variants + reports; account deletion uses the cancellable seven-day notice and joined-object purge lifecycle; no fee path | 🟡 | `e2e/deletion-export.spec.ts` verifies the free export and cancellable notice period. `e2e/account-deletion-purge.spec.ts` now proves the supported self-account flow through exact joined Storage deletion, relational zero-residual checks, Auth-user-last deletion, and pseudonymized terminal control rows. Complex adult/family/embryo/claim/reviewer graphs deliberately fail closed until every disposition in the canonical registry is implemented, so this remains 🟡 rather than overstating full-contract coverage. No billing/fee code path exists. |
