...
test("explicit VCF reference calls match array findings without entering variant-only analyses", async ({ page }) => {
  ...
  for (const kind of ["vcf", "array_23andme"] as const) {
    ...
    writeFileSync(fixture, (kind === "array_23andme" ? "# This data file generated by 23andMe\n" : "")
      + readFileSync(original, "utf8").trimEnd() + "\n" + additions[extension]);
    const id = await uploadOwnFileWithChosenReports(page, fixture, { fileType: kind, purposes: ["reports.polygenic"] });
    if (kind === "vcf") {
      const { data: file, error } = await admin.from("genome_files").select("sha256,upload_revision,normalization_source_revision,normalization_completed_at,variant_count").eq("id", id).single();
      expect(error).toBeNull();
      const sourceHash = createHash("sha256").update(readFileSync(fixture)).digest("hex");
      expect(file).toMatchObject({ sha256: sourceHash, variant_count: 0 });
      expect(file!.normalization_source_revision).toBe(file!.upload_revision);
      expect(file!.normalization_completed_at).not.toBeNull();
      ...
      for (const call of before.data!) {
        expect(call).toMatchObject({ source_sha256: sourceHash, extraction_version: "vcf-literal-diploid-snp-v1", usable: true });
      }
      expect((await admin.from("user_variants").select("id").eq("file_id", id)).data).toEqual([]);
      const reprocessed = await page.evaluate(async (fileId) => {
        const response = await fetch(`/api/files/${fileId}/process`, { method: "POST" });
        return { status: response.status, body: await response.json() };
      }, id);
      expect(reprocessed.status).toBe(200);
      expect(reprocessed.body).toMatchObject({ fileId: id, analysisState: "active" });
