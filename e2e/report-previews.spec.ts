...
for (const [gt, keys] of [
  ...
  test(`everyday report upload ${gt} binds the actual result to its sources and personal preview`, async ({ page }) => {
    ...
    try {
      ...
      await uploadOwnFileWithChosenReports(page, file, { fileType: "vcf", purposes: ["reports.polygenic"] });
...
for (const [genotype, gt] of [["GG", "0/0"], ["AA", "1/1"]] as const) {
  test(`alcohol ${genotype} upload shows the matching takeaway and qualified full result`, async ({ page }) => {
    ...
    try {
      await createConfirmedUser(user.email, user.password);
      await signIn(page, user.email, user.password);
      await uploadOwnFileWithChosenReports(page, fixture, { fileType: "vcf", purposes: ["reports.polygenic"] });
      ...
      if (genotype === "GG") {
        ...
        fs.writeFileSync(arrayFixture, "# This data file generated by 23andMe\n# Synthetic test data; not a real person\n# reference build 38\n# rsid\tchromosome\tposition\tgenotype\nrs671\t12\t111803962\tGG\n");
        await uploadOwnFileWithChosenReports(page, arrayFixture, { fileType: "array_23andme", purposes: ["reports.polygenic"] });
        await page.goto("/genome/me/reports");
      }
...
test("own DNA shows four useful takeaways, filters all results, and links to the source-backed reports", async ({ page }) => {
  await createConfirmedUser(USER.email, USER.password);
  await signIn(page, USER.email, USER.password);
  const fileId = await uploadOwnFileWithChosenReports(page, FIXTURE, { fileType: "vcf", purposes: ["reports.polygenic"] });
  await page.goto("/genome/me/reports");
  await expect(page.locator("[data-personal-preview]")).toHaveCount(4);
