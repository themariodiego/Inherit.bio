...
async function ask(page: Page, fixture: CopilotFixture, prompt: string, slug: string, answer: string) {
  await fixture.configure({ prompt, tool: { name: "get_report", arguments: { slug } }, answer });
...
for (const scenario of REGIONAL_FIXTURES) {
  test(`own genome from ${scenario.merged ? "Overview" : "My Genome"}: one file, reports, ancestry and Copilot`, async ({ page }) => {
    ...
    try {
      ...
      const prepared = page.waitForResponse(response => /\/api\/files\/[0-9a-f-]{36}\/process$/.test(response.url())
        && response.request().method() === "POST");
      for (const promise of [finalized, prepared]) void promise.catch(() => {});
      ...
      await openGenomeTool(page, "Copilot");
      const report = await ask(page, fixture, CAFFEINE_PROMPT, CAFFEINE_SLUG, CAFFEINE_ANSWER);
      ...
      const ancestry = await ask(page, fixture, "Explain my displayed ancestry estimate.", "inherit:ancestry", answer);
      ...
      expect((await fixture.snapshot()).calls).toBe(4);
      ...
      expect(saved.messages[3]).toMatchObject({ content: answer, citations });
      await expect(page.getByText(answer, { exact: true })).toBeVisible();
      expect((await fixture.snapshot()).calls).toBe(4);
