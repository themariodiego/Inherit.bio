...
## Five focused invalidation cases

`canonical-copilot-invalidation.spec.ts` adds five real browser cases (73 together with the original 68): two genuinely prepared current sources disagreeing at a locus, an absent locus and an ungenerated report; report withdrawal before returning the tool call; source deletion after the tool result; Copilot-purpose withdrawal after the tool result followed by genuine regrant; and model-configuration change after the tool result. Existing source fixtures are preserved. No legacy-normalized file or grant is fabricated.

Each interrupted request must produce only the closed 403 error, without the synthetic final answer or a new persisted user/assistant half. Existing conversations become 404 under their stale source/authority projection. Report withdrawal's actual synchronous exact-grant executor must physically delete the original two message IDs and leave matching complete frozen-manifest/disposition evidence. Copilot-purpose/model changes queue their exact old-grant membership; those tests account for the old paired rows while verifying immediate invisibility. Source deletion must remove its original and the exact dependent pair IDs while preserving the other original's exact bytes. Its private pending-deletion manifest cascades away at finalization, so the browser checks exact pre/post message membership rather than claiming to inspect that finished private record. Regrant must use the new real permission and cannot reopen old history.

The fixture never calls global `run_own_report_purge_v1` or `/api/jobs/retention`: neither accepts an exact test-owned selector. Physical completion of queued Copilot-purpose work needs the separately controlled disposable SQL/worker verification, and is not claimed by these browser assertions.
