...
## density-source-grch38.vcf

- Classification: synthetic single-sample GRCh38 VCF, the source the
  post-change density capture uploads (G2.5). It describes no real person: it
  is the union of every position of the shipped ancestry marker panel
  (`data/ref/aims.json`) and every variant position the shipped report
  catalogue names (`data/templates/*.json`), 309 rows, with every genotype
  drawn by a seeded pseudo-random generator (mulberry32, seed 1). No genotype
  was read from any person or sample.
- Why a third fixture rather than one of the two above. The post-change
  capture has to put each successor page in the state its predecessor was
  measured in, or the comparison measures a STATE change and reports it as a
  density change. `aims-mixed-grch38.vcf` carries the panel and no report
  locus, so the report library and the one report the baseline measured would
  render an absence; `tiny-grch38.vcf` carries a handful of loci and one
  usable ancestry marker, which renders the grey map.
- The ancestry half is drawn by the same generator and the same mixing
  weights as `aims-mixed-grch38.vcf`, so the map it produces is one that
  fixture's own checks already constrain. Where the panel and the catalogue
  name the same position — five of them — the ancestry draw wins: the
  estimator reads every panel position and a report reads one.
- chr10:112998590 is required to carry an ALT allele, because
  `/reports/type-2-diabetes-tcf7l2-rs7903146` is the report the frozen
  baseline measured and its successor must be able to show one. The seed is
  searched upward from 1 until the draw both shows the map and carries that
  allele; seed 1 does both, at 97 usable markers against a minimum of 42.
- Generated by `pnpm exec tsx e2e/fixtures/generate-density-source-vcf.ts`,
  which rewrites the file byte-identically and refuses to write it unless
  both of those properties hold. `scripts/density-source-fixture.test.ts`
  rebuilds the committed bytes from the generator and compares them, so a
  catalogue or panel change that moves this fixture fails rather than
  silently changing what the capture measures.
- Repository SHA-256:
