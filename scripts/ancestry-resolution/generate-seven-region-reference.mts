...
const { values: args } = parseArgs({ options: {
  check: { type: "boolean" }, frequencies: { type: "string" }, populations: { type: "string" },
} });
const read = (file: string) => JSON.parse(fs.readFileSync(path.resolve(ROOT, file), "utf8")) as unknown;
const sources = {
  markers: read("data/ref/aims.json"),
  frequencies: read(args.frequencies ?? "scripts/ancestry-resolution/hgdp-tgp-freqs.json"),
  populations: read(args.populations ?? "scripts/ancestry-resolution/hgdp-tgp-populations.json"),
};
...
const runtimeModule = "/** Generated by generate-seven-region-reference.mts; preserve exact numbers via native JSON.parse. */\n"
  + `export const REGIONAL_REFERENCE_JSON = ${JSON.stringify(JSON.stringify(table))};\n`;
...
const outputs = [
  ...
  ["src/lib/genome/regional-reference-json.ts", runtimeModule],
] as const;
if (args.check) {
