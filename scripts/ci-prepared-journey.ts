...
export async function withPreparedJourney<T>(env: Environment, work: (fixture: {
  ...
  try {
    const activated = await sql(`with changed as (update ${CONFIG} c set enabled=true where singleton and to_jsonb(c)=${jsonLiteral(original)} returning 1) select count(*) from changed;`);
    ...
    return await work({
      ...
      proof: async fileId => JSON.parse(await sql(preparedProof(fileLiteral(fileId)))),
    });
  } finally {
    ...
    const restored = await sql(`update ${CONFIG} set enabled=false where singleton; select to_jsonb(c) from ${CONFIG} c where singleton;`);
    assert.deepEqual(checkedConfig(JSON.parse(restored)), original, "Preparation configuration did not restore exactly");
  }
}

function preparedProof(file: string): string {
  return `select json_build_object(
    ...
    'membersCurrent',coalesce((select m.member_count=count(mm.artifact_id) and bool_and(a.state='acknowledged' and a.observed_sha256=a.sha256 and a.provider='supabase' and a.provider_bucket='genomes')
