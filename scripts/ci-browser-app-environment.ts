...
import { APP_ENV_NAMES, APP_PORTS, LOCAL_MODEL_ENV, LOCAL_MODEL_PORT } from "./ci-browser-config";
 ...
 * the fixed attestation in `LOCAL_MODEL_ENV`. Only the variant fields are
...
export function appServerEnvironment(env: Environment, port: AppPort): Record<string, string> {
  ...
  return port === LOCAL_MODEL_PORT ? { ...shared, ...LOCAL_MODEL_ENV } : shared;
}
